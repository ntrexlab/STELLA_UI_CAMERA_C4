...
from stella_ui_camera.msg import Robot_status
...
class MyFrame(rtmgr.MyFrame):
	...
	def __init__(self, *args, **kwds):
		...
		self.pre_v_message = 0
		self.t = threading.Thread(target = self.teleoperation)
		self.t_robot = threading.Thread(target = self.robot_status)
		self.target_linear_vel   = 0.0
		self.target_angular_vel  = 0.0

		self.robot_cur_x = 0.0
		self.robot_cur_y = 0.0
		self.robot_goal_x = 0.0
		self.robot_goal_y = 0.0
		self.robot_linear_speed = 0.0
		self.robot_angular_speed = 0.0
		
		self.robot_cur_x_msg = 0.0
		self.robot_cur_y_msg = 0.0
		self.robot_goal_x_msg = 0.0
		self.robot_goal_y_msg = 0.0
		self.robot_linear_speed_msg = 0.0
		self.robot_angular_speed_msg = 0.0


		#
		# ros
		#
		rospy.init_node('stella_ui', anonymous=True)
		...
		self.t_robot.start()
		rospy.Subscriber('amcl_pose', PoseWithCovarianceStamped, self.robot_status_data_position)
		rospy.Subscriber('move_base_simple/goal', PoseStamped, self.robot_status_data_goal)
		rospy.Subscriber('cmd_vel', Twist, self.robot_status_data_vel)
		...
		self.status_curr_topic = '/robot_status'
		self.status_proc = None
		self.status_thinf = None

		self.status_que = queue.Queue()
		self.status_sum = 0
		status_thinf = th_start(self.status_show_th)
		self.all_th_infs.append(status_thinf)
		
		if self.load_dic.get('connection_data') != None:
			self.label_remote_ip.SetValue(self.load_dic.get('connection_data').get('remote_ip'))
			self.label_robot_ip.SetValue(self.load_dic.get('connection_data').get('robot_ip'))
			self.label_robot_id.SetValue(self.load_dic.get('connection_data').get('robot_id'))
			self.label_robot_password.SetValue(self.load_dic.get('connection_data').get('robot_password'))
...
#                        post_evt_toggle_obj(self, obj, True)
	...
	def save_param_yaml(self):
		...
		remote_ip = self.label_remote_ip.GetValue()
		robot_ip = self.label_robot_ip.GetValue()
		robot_id = self.label_robot_id.GetValue()
		robot_password = self.label_robot_password.GetValue()
		if 'connection_data' not in save_dic:
			save_dic['connection_data'] = {}
		save_dic.get('connection_data')['remote_ip'] = str(remote_ip)
		save_dic.get('connection_data')['robot_ip'] = str(robot_ip)
		save_dic.get('connection_data')['robot_id'] = str(robot_id)
		save_dic.get('connection_data')['robot_password'] = str(robot_password)
...
#		return (enable, msg)
	...
	def OnConnect(self, event):
		...
		if v == True:
			self.robot_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source /opt/ros/melodic/setup.bash && source ~/catkin_ws/devel/setup.bash && export ROS_MASTER_URI=http://' + remote_ip +':11311 && export ROS_HOSTNAME=' + robot_ip + ' && roslaunch stella_bringup stella_robot.launch'], stdout=out, stderr=err)
		elif v == False:
	...
	def OnSlam(self, event):
		...
		if v == True:
			...
			self.robot_slam_proc = psutil.Popen("gnome-terminal --tab -- roslaunch stella_slam stella_slam.launch && sleep 2 && wmctrl -r 'stella_cartographer.rviz - RViz' -e 0,0,180,630,370",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
			
		elif v == False:
			self.button_save_map.Disable()
			self.robot_slam_proc = psutil.Popen("killall -9 rviz",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
	...
	def OnSaveMap(self, event):
		...
		self.path_save_map = self.load_dic.get('save_map').get('path_save_map')

		if v == True:
			self.robot_slam_proc = psutil.Popen("rosrun map_server map_saver -f " + self.path_save_map + "/map",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
			terminate_children(self.robot_slam_proc)
			terminate(self.robot_slam_proc)
			self.robot_slam_proc.wait()
			self.robot_slam_proc = None
	...
	def OnNavigation(self, event):
		...
		if v == True:
			self.robot_nav_proc = psutil.Popen("gnome-terminal --tab -- roslaunch stella_navigation stella_navigation.launch map_file:=" + self.path_navigation + "&& sleep 2 && wmctrl -r 'stella_navigation.rviz - RViz' -e 0,0,180,630,370",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
			self.robot_nav_proc = psutil.Popen("killall -9 rviz",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)

	def OnCameraConnect(self,event):
		v = (event.GetEventObject()).GetValue()
		out = subprocess.PIPE
		err = subprocess.STDOUT
		...
		if v == True:
			self.robot_camera_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source /opt/ros/melodic/setup.bash && source ~/catkin_ws/devel/setup.bash && export ROS_MASTER_URI=http://' + remote_ip +':11311 && export ROS_HOSTNAME=' + robot_ip + ' && roslaunch stella_camera stella_camera.launch'], stdout=out, stderr=err)
		elif v == False:
			self.robot_camera_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source /opt/ros/melodic/setup.bash && source ~/catkin_ws/devel/setup.bash && export ROS_MASTER_URI=http://' + remote_ip +':11311 && export ROS_HOSTNAME=' + robot_ip + ' && rosnode kill stella_camera_node'], stdout=out, stderr=err)

	def OnCameraShow(self,event):
		v = (event.GetEventObject()).GetValue()
		out = subprocess.PIPE
		err = subprocess.STDOUT
		...
		if v == True:
			self.robot_cam_proc = psutil.Popen("gnome-terminal --tab -- rqt_image_view /camera && sleep 2 && wmctrl -r 'rqt_image_view__ImageView - rqt' -e 0,0,550,630,370",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
			
		elif v == False:
			self.robot_cam_proc = psutil.Popen("killall -9 rqt_image_view",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
	...
	def OnTeleoperation(self, event):
		push = event.GetEventObject()
		if push == self.button_teleoperation:
			v = self.button_teleoperation.GetValue()			
			if v == True:
				self.target_linear_vel = 0.0
				self.target_angular_vel = 0.0
				self.t.start()
				set_val(self.button_teleoperation,True)
				self.button_W.Enable()
				self.button_A.Enable()
				self.button_S.Enable()
				self.button_D.Enable()
				self.button_X.Enable()
				
			elif v == False:
				self.t.join()
				self.t = threading.Thread(target = self.teleoperation)		
				set_val(self.button_teleoperation,False)
		...
		elif push == self.button_W:
			self.target_linear_vel = self.target_linear_vel + 0.1
			if self.target_linear_vel > 2:
				self.target_linear_vel  = 2

		elif push == self.button_A:
			self.target_angular_vel = self.target_angular_vel +0.1
			if self.target_angular_vel > 3.14:
				self.target_angular_vel = 3.14

		elif push == self.button_S:
			self.target_linear_vel = 0.0
			self.target_angular_vel = 0.0

		elif push == self.button_D:
			self.target_angular_vel = self.target_angular_vel -0.1
			if self.target_angular_vel < -3.14:
				self.target_angular_vel = -3.14

		elif push == self.button_X:
			self.target_linear_vel = self.target_linear_vel - 0.1
			if self.target_linear_vel < -2:
				self.target_linear_vel = -2
	...
	def robot_status(self):
		while not rospy.is_shutdown():
			r = rospy.Rate(10)
			pub = rospy.Publisher('robot_status', Robot_status, queue_size = 1)
			robot_status = Robot_status()
			robot_status.current_x = self.robot_cur_x
			robot_status.current_y = self.robot_cur_y
			robot_status.goal_x = self.robot_goal_x
			robot_status.goal_y = self.robot_goal_y
			robot_status.linear_speed = self.robot_linear_speed
			robot_status.angular_speed = self.robot_angular_speed
			pub.publish(robot_status)
			r.sleep()

	def robot_status_data_position(self,data):
		self.robot_cur_x = data.pose.pose.position.x
		self.robot_cur_y = data.pose.pose.position.y

	def robot_status_data_goal(self,data):
		self.robot_goal_x = data.pose.position.x
		self.robot_goal_y = data.pose.position.y
	
	def robot_status_data_vel(self,data):
	...
	def OnMessage(self, event):
		...
		grp_course = { 
			self.button_message_on : 1,
			self.button_message_off : 2 }
		self.radio_action(event, grp_course.keys())
		v_message = grp_course.get(event.GetEventObject())
		
		if v_message == 1:
			if self.pre_v_message == 2 or self.pre_v_message == 0:
				self.status_proc_th_start(self.status_curr_topic)
				self.pre_v_message = v_message
		
		if v_message == 2:
			if self.pre_v_message == 1 or self.pre_v_message == 0:
				self.status_proc_th_end()
				self.pre_v_message = v_message
...
#				time.sleep(0.05)
	...
	# for Map tab
	#
	def status_proc_th_start(self, topic):
		out = subprocess.PIPE
		err = subprocess.STDOUT
		self.status_proc = psutil.Popen([ 'rostopic', 'echo', topic ], stdout=out, stderr=err)

		self.status_thinf = th_start(self.status_th)
	
	def status_proc_th_end(self):
		thinf = self.status_thinf
		if thinf:
			th_end(thinf)
			self.status_thinf = None

		proc = self.status_proc
		if proc:
			terminate_children(proc)
			terminate(proc)
			#proc.wait()
	...
	def status_th(self, ev):
		if not self.status_proc:
			return
		file = self.status_proc.stdout
		fl = fcntl.fcntl(file.fileno(), fcntl.F_GETFL)
		fcntl.fcntl(file.fileno(), fcntl.F_SETFL, fl | os.O_NONBLOCK)
		...
		while not ev.wait(0):
			...
			self.status_que.put(s)
		que_clear(self.status_que)

	def status_show_th(self, ev):
		interval = 0.1
		chars_limit = 1500
		tc = self.label_mobile_robot
		font = wx.Font(17, wx.NORMAL, wx.NORMAL, wx.NORMAL)
		tc.SetFont(font)
		while not ev.wait(interval):
			s = "current_x :" + str(self.robot_cur_x_msg) + "\n" + "current_y :" + str(self.robot_cur_y_msg) + "\n"+ "goal_x :" + str(self.robot_goal_x_msg) + "\n" + "goal_y:" + str(self.robot_goal_y_msg) + "\n" + "linear_speed :" + str(self.robot_linear_speed_msg) + "\n" + "angular_speed :" + str(self.robot_angular_speed_msg)

			self.status_sum += len(s)
			rm_chars = 0
			if self.status_sum > chars_limit:
				rm_chars = self.status_sum - chars_limit
				self.status_sum = chars_limit
...
#			self.stat_label_off(obj)
	...
	def launch_kill(self, v, cmd, proc, add_args=None, sigint=None, obj=None, kill_children=None):
		...
		if v:
			...
			if shell:
				args = ' '.join( args[1:] )
			args = 'rosrun map_server map_saver -f ~/map'
